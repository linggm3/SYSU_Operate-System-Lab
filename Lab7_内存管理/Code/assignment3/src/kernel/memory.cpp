...
void MemoryManager::initialize()
{
    ...
    kernel_queue.initialize();
    user_queue.initialize();
    /*// kernel_virtual_bitmap_start 往后 ceil(kernelPages/8) 个 字节 存放 kernel_LRU_start
    int* kernel_LRU_start = (int*)(kernelVirtualBitMapStart + ceil(kernelPages, 8));
    // kernel_LRU_start 往后 kernelPages * 32 个 字节 存放 user_LRU_start
    int* user_LRU_start = (int*)(kernelVirtualBitMapStart + kernelPages * 32);
    for(int i = 0; i < kernelPages; i++){
        kernel_LRU_start[i] = 0;
        printf("%x\n", &kernel_LRU_start[i]);
    }
...
int MemoryManager::allocatePages(enum AddressPoolType type, const int count)
{
    ...
    for (int i = 0; i < count; ++i, vaddress += PAGE_SIZE)
    {
        ...
        // 分配彻底失败（不可能出现），释放前面已经分配的虚拟页和物理页表
        if (!flag)
        {
